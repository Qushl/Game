...
namespace TopDownHighwayDrifter
{
    ...
    public class RoadPath
    {
        ...
        private void GeneratePath()
        {
            ...
            for (int i = 0; i < 30; i++) AddSegment(ref currentDistance, ref currentAngle);
        }
        ...
        public void AddSegment(ref float currentDistance, ref float currentAngle)
        {
            ...
            var segment = new RoadSegment { StartDistance = currentDistance, Length = segmentLength, StartAngle = currentAngle, EndAngle = currentAngle + turnAngle };
        ...
        {
            while (_segments.Count > 0 && _segments[0].StartDistance + _segments[0].Length < minDistance) _segments.RemoveAt(0);
        }
        ...
        public RoadInfo GetRoadInfoAtDistance(float distance)
        {
            ...
            foreach (var segment in _segments)
            {
                if (distance >= segment.StartDistance && distance < segment.StartDistance + segment.Length)
                {
                    ...
                    return new RoadInfo { Distance = distance, Angle = angle, Segment = segment, SegmentProgress = t };
                }
            }
            ...
            if (_segments.Count > 0)
            {
                ...
                return new RoadInfo { Distance = distance, Angle = lastSeg.EndAngle, Segment = lastSeg, SegmentProgress = 1.0f };
            }

            return new RoadInfo { Distance = distance, Angle = 0, Segment = null, SegmentProgress = 0 };
        }
        ...
        public void GetWorldPosition(float distance, out float worldX, out float worldY, out float angle)
        {
            ...
            if (_segments.Count > 0) angle = _segments[^1].EndAngle;
            worldX = currentX;
            worldY = currentY;
        }
    }
